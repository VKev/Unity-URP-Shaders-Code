struct appdata
            ...
            //***************************************  
            float3 windAnimation(vertOut o,float3 modifiedPos , float normalRandom, float waveLocalHorizontalAmplitude,float waveLocalVerticalAmplitude){
                    modifiedPos += WindHorizontalOS( o.uv,normalRandom, _WaveLocalSpeed, waveLocalHorizontalAmplitude, _WaveLocalDir);
                    modifiedPos += WindVerticalOS( o.uv, normalRandom, _WaveLocalSpeed, waveLocalVerticalAmplitude);



                    float3 finalPositionWS = GetVertexPositionInputs(modifiedPos).positionWS;
            ...
            half4 frag(vertOut i, FRONT_FACE_TYPE frontFace : FRONT_FACE_SEMANTIC) : SV_Target//get front face of object
            {
                ...
                if(mainLight.shadowAttenuation <0.5)
                    mainCol += AmbientCol.rgb*mainLight.color;
